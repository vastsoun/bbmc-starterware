...
/* Driver Constants */
#define DMTIMER_05MS                    (0xFFFFD11Fu)
#define DMTIMER_1MS                     (0xFFFFA23Fu)
#define DMTIMER_2MS                     (0xFFFF447Fu)
#define DMTIMER_10MS                    (0xFFFC567Fu)

#define DMTIMER_COUNT_MAX               (0xFFFFFFFFu)
#define DMTIMER_SYSTEM_CLK_DEFAULT      (24000000)
...
/**
 * \struct  dmtimer_device_t
 * \brief   am335x-dmtimer device registers
 * 
...
{
    hw_devreg32_t tidr;
    hw_devreg32_t padding0[3];
    hw_devreg32_t tiocp_cfg;
    hw_devreg32_t padding1[3];
    hw_devreg32_t irq_eoi;
    hw_devreg32_t irqstatus_raw;
    hw_devreg32_t irqstatus;
    hw_devreg32_t irqenable_set;
    hw_devreg32_t irqenable_clr;
    hw_devreg32_t irqwakeen;
    hw_devreg32_t tclr;
    hw_devreg32_t tcrr;
    hw_devreg32_t tldr;
    hw_devreg32_t ttgr;
...
/**
 * \struct  dmtimer_handle_t
 * \brief   am335x-dmtimer device handle
 * 
 * This data struct is wraps the interface to the device registers of the
 * peripheral hardware.
 * 
 * \param device_id The id-tag that designates the dmtimer instance to be handled
 * 
...
typedef struct
{
    ...
    dmtimer_device_t config;
} 
dmtimer_handle_t;


/**
 * \struct  dmtimerContext
 * \brief   dmtimer context struct
 * 
 * This data struct is wraps the interface to the device registers of the
 * peripheral hardware.
 * 
...
extern int dmtimer_open (unsigned int device_id);

extern int dmtimer_close (unsigned int device_id);

extern int dmtimer_init (unsigned int device_id);

extern int dmtimer_enable (unsigned int device_id);

extern int dmtimer_disable (unsigned int device_id);

extern int dmtimer_counter_reset (unsigned int device_id);

extern int dmtimer_config_set (dmtimer_handle_t *handle);

extern int dmtimer_config_get (dmtimer_handle_t *handle);


/* Specialized Functions */

extern int dmtimer_handle_init (unsigned int device_id, dmtimer_handle_t *handle);

extern int dmtimer_tldr_config_set (unsigned int device_id, unsigned int load_value);

extern unsigned int dmtimer_tldr_config_get (unsigned int device_id);
